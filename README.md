# Surabi Gelo — Rilis APK

Repo **publik** ini hanya menampung berkas APK aplikasi kasir Surabi Gelo.
Kode sumbernya ada di repo privat `pos-surabi-gelo-mobile` dan **tidak** ada di sini.

Kenapa terpisah: tablet mengunduh pembaruan tanpa login, jadi berkasnya harus
bisa diakses publik. Membuat repo kode jadi publik hanya demi ini tidak sepadan
risikonya.

Aplikasi mengecek `latest.json` di Supabase Storage (bucket `app-releases`),
lalu mengunduh APK dari sini.

| Versi | Build | Berkas |
|---|---|---|
| 1.5.2 | 98 | `surabi-gelo-98.apk` (BELUM dirilis — latest.json masih 96; menggantikan 97) |
| 1.5.0 | 96 | `surabi-gelo-96.apk` |
| 1.4.9 | 95 | `surabi-gelo-95.apk` (BELUM dirilis — latest.json masih 92) |
| 1.4.8 | 94 | `surabi-gelo-94.apk` (BELUM dirilis — latest.json masih 92) |
| 1.4.7 | 93 | `surabi-gelo-93.apk` (BELUM dirilis — latest.json masih 92) |
| 1.4.6 | 92 | `surabi-gelo-92.apk` (BELUM dirilis — latest.json masih 90) |
| 1.4.5 | 91 | `surabi-gelo-91.apk` (BELUM dirilis — latest.json masih 90) |
| 1.4.4 | 90 | `surabi-gelo-90.apk` |
| 1.4.3 | 89 | `surabi-gelo-89.apk` |
| 1.4.2 | 88 | `surabi-gelo-88.apk` |
| 1.4.1 | 87 | `surabi-gelo-87.apk` |
| 1.4.0 | 86 | `surabi-gelo-86.apk` |
| 1.3.9 | 85 | `surabi-gelo-85.apk` |
| 1.3.8 | 84 | `surabi-gelo-84.apk` |
| 1.3.7 | 83 | `surabi-gelo-83.apk` |
| 1.3.6 | 82 | `surabi-gelo-82.apk` |
| 1.3.5 | 81 | `surabi-gelo-81.apk` |
| 1.3.4 | 80 | `surabi-gelo-80.apk` |
| 1.3.3 | 79 | `surabi-gelo-79.apk` |
| 1.3.2 | 78 | `surabi-gelo-78.apk` |
| 1.3.1 | 77 | `surabi-gelo-77.apk` |
| 1.3.0 | 76 | `surabi-gelo-76.apk` |
| 1.2.8 | 75 | `surabi-gelo-75.apk` |
| 1.2.7 | 74 | `surabi-gelo-74.apk` |
| 1.2.6 | 73 | `surabi-gelo-73.apk` |
| 1.2.5 | 72 | `surabi-gelo-72.apk` |
| 1.2.4 | 71 | `surabi-gelo-71.apk` |
| 1.2.3 | 70 | `surabi-gelo-70.apk` |
| 1.2.2 | 69 | `surabi-gelo-69.apk` |
| 1.2.1 | 68 | `surabi-gelo-68.apk` |
